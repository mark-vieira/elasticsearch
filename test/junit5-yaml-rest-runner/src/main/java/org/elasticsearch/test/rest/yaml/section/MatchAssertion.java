...
import static org.elasticsearch.test.matcher.ListMatcher.matchesList;
import static org.elasticsearch.test.matcher.MapMatcher.assertMap;
import static org.elasticsearch.test.matcher.MapMatcher.matchesMap;
...
public class MatchAssertion extends Assertion {
    ...
    private static final Logger logger = LoggerFactory.getLogger(MatchAssertion.class);
    ...
    protected void doAssert(Object actualValue, Object expectedValue) {
        ...
        if (expectedValue instanceof String) {
            ...
            if (expValue.length() > 2 && expValue.startsWith("/") && expValue.endsWith("/")) {
                ...
                logger.trace(() -> "assert that [" + stringValue + "] matches [" + regex + "]");
                assertThat(
        ...
        logger.trace(() -> "assert that [" + actualValue + "] matches [" + expectedValue + "] (field [" + getField() + "])");
        if (expectedValue == null) {
            assertNull(actualValue, "field [" + getField() + "] should be null but was [" + actualValue + "]");
            return;
        }
        assertNotNull(actualValue, "field [" + getField() + "] is null");
