...
public class ClientYamlTestClient implements Closeable {
    private static final Logger logger = LoggerFactory.getLogger(ClientYamlTestClient.class);
    ...
    private final Supplier<RestClientBuilder> clientBuilderWithSniffedNodes;
    ...
    public ClientYamlTestClient(
        ...
        final String os,
        final Supplier<RestClientBuilder> clientBuilderWithSniffedNodes
    ) {
        assert hosts.size() > 0;
    ...
    public ClientYamlTestResponse callApi(
        ...
        ClientYamlSuiteRestApi.Path path = bestPaths.get(0);
        ...
        if (entity != null) {
            ...
            if (sendBodyAsSourceParam(supportedMethods, contentType, entity)) {
                ...
                requestMethod = supportedMethods.get(0);
            }
        } else {
        ...
        for (Entry<String, String> param : queryStringParams.entrySet()) {
    ...
    public RestClient getRestClient(NodeSelector nodeSelector) {
    ...
    protected static void setOptions(Request request, Map<String, String> headers) {
        ...
        for (Entry<String, String> header : headers.entrySet()) {
            logger.debug(() -> "Adding header " + header.getKey() + " with value " + header.getValue());
