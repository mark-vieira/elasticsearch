...
public class SkipSection {
    ...
    public static SkipSection parse(XContentParser parser) throws IOException {
            ...
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if ("version".equals(currentFieldName)) {
                    ...
                    throw new IllegalStateException(
                        "field " + currentFieldName + " not supported within skip section @ " + parser.getTokenLocation().toString()
                    );
                }
        ...
        if ((StringUtils.isBlank(version)) && features.isEmpty() && operatingSystems.isEmpty()) {
            throw new IllegalStateException(
                "version, features or os is mandatory within skip section @ " + parser.getTokenLocation().toString()
            );
        }
        if (StringUtils.isNotBlank(version) && StringUtils.isBlank(reason)) {
            throw new IllegalStateException("reason is mandatory within skip version section @ " + parser.getTokenLocation().toString());
        }
        if (operatingSystems.isEmpty() == false && StringUtils.isBlank(reason)) {
            throw new IllegalStateException("reason is mandatory within skip version section @ " + parser.getTokenLocation().toString());
        }
        ...
        if (operatingSystems.isEmpty() == false && features.contains("skip_os") == false) {
            throw new IllegalStateException("if os is specified, feature skip_os must be set @ " + parser.getTokenLocation().toString());
        }
    ...
    static List<VersionRange> parseVersionRanges(String rawRanges) {
        ...
        if (rawRanges.trim().equals("all")) {
            return Collections.singletonList(new VersionRange(Version.FIRST, Version.CURRENT));
        }
        ...
        for (String rawRange : ranges) {
            ...
            VersionRange versionRange = new VersionRange(
                lower.isEmpty() ? Version.FIRST : Version.fromString(lower),
